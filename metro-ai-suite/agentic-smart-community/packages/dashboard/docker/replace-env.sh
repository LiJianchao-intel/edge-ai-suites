...
require_env SMARTHOME_API_PORT
require_env STATS_API_PORT
require_env RAG_API_PORT
require_env RAG_CHAT_PORT
...
envsubst '${UI_PORT} ${SERVER_HOST} ${SMARTHOME_API_PORT} ${STATS_API_PORT} ${RAG_API_PORT} ${RAG_CHAT_PORT} ${VITE_CHATBOT_WS_PORT}' \
    < "$NGINX_TEMPLATE" \
    > "$NGINX_CONF"
